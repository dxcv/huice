...
from vtObject import VtTickData, VtBarData, VtOrderData, VtTradeData
...
class BacktestingEngine(object):
    ...
    def loadHistoryData(self):
        ...
        if self.hdsClient:
            self.dbCursor = self.hdsClient.loadHistoryData(self.dbName,
                                                           ...
                                                           self.dataEndDate)
        # 从本地数据库导入数据
        # 注意与初始化的区别，初始化的开始日期为DateStartDate,而下列数据的加载日期
        # 为strategyStartDate
        else:
            # 判断是否有回测结束日期，若无就代表回测结束时间为到目前为止
    ...
    def loadHistoryDataList(self):
        ...
        for s in self.symbol:
            collection = self.dbClient[self.dbName][s]
            self.output(u'开始载入%s数据' % s)
            # 首先根据回测模式，确认要使用的数据类
            if self.mode == self.BAR_MODE:
                dataClass = VtBarData
                func = self.newBar
            ...
            for d in initCursor:
                ...
                data.__dict__ = d
                self.initData.append(data)
            # self.allInitData.append(self.initData)
            ...
            if isinstance(self.dbCursor, list):
                ...
                count = initCursor.count() + self.dbCursor.count()
            totalCount += count
        self.initData.sort(key=lambda x: x.datetime)
        self.huicedata.sort(key=lambda x: x.datetime)
        self.output(u'载入完成，数据量：%s' % totalCount)
    #----------------------------------------------------------------------
